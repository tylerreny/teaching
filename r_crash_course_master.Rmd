...
Sidebar: What is a factor? Also note: these are some of the first steps I take when looking at data.

```{r}
#lets load some sample data
library(gapminder) #this is the dataset
...
gm2 <- na.omit(gapminder) #listwise delete the row
nrow(gm2)
nrow(gapminder)
```
...
test <- c("This", "Class", "Is", "Fun", "Fun", "Fun")
unique(test)
```
...
```{r}
#side note on packages and overwriting functions
...
This is totally useless and what Kathy Bryant calls Data Puke. Need to re-order:
...
If you have a dataframe and you want to sort it, use the order function:
...
One way is called listwise deletion, which I introduced above -- it basically will just delete all NAs in a vector (but be careful, it will delete complete rows in a dataframe! (SIDE NOTE ON REGRESSION AND LISTWISE DELETE)
...
df <- read.csv("/Users/treny/cleaned_anes_pilot_2016.csv", stringsAsFactors = F)
data.out <- df %>% dplyr :: select(vote12, gender, white, pid7, ideo5, rr_scale)
write.csv(data.out)

data.out$vote_obama <- ifelse(data.out$vote12 == 2, 1, 0)
data.out$white <- as.numeric(data.out$white)
write.table(data.out, "/Users/treny/Dropbox/_projects/r_crash_course/sample_anes_data.txt", sep="\t")
...
head(data.out)
summary(glm(vote_obama ~ gender + white + pid7 + ideo5 + rr_scale, family=binomial(link='logit'), data=data.out))

glm.out <- glm(vote_obama ~ gender + white + pid7 + ideo5 + rr_scale, family=binomial(link='logit'), data=data.out)
pe <- coef(glm.out)
vc <- vcov(glm.out)
simbetas <- mvrnorm(1000, pe, vc)

hyp.x <- seq(1, 5, 0.25)
hyp.data <- cfMake(vote_obama ~ gender + white + pid7 + ideo5 + rr_scale, data.out, nscen=length(hyp.x))
hyp.data <- cfChange(hyp.data, "rr_scale", x=hyp.x, scen=1:length(hyp.x))
logit.out <- logitsimev(hyp.data, simbetas, ci=0.95)
logit.out

datafp <- data.frame(x=hyp.x,
                     y=logit.out$pe,
                     ylower=logit.out$lower,
                     yupper=logit.out$upper)
ggplot(datafp, aes(x=x, y=y)) + geom_line(color="red") + 
  theme_bw() + labs(x='Racial Resentment', y="Prob Voting Obama 2012") +
  geom_ribbon(aes(ymin=ylower, ymax=yupper), alpha=.1)
...
url <- ("https://raw.githubusercontent.com/tylerreny/teaching/master/sample_anes_data.txt")
df <- source_GitHubData(url, sep='\t')


library('httr')    
raw <- content(GET("https://raw.githubusercontent.com/tylerreny/teaching/master/sample_anes_data.txt"))
read.table(raw, sep='\t')

data.out <- data.out %>% head(1000)
data.out %>% arrange(desc(ftobama)) %>% head()

url <- url()
test <- read.table.url(, sep="\t")
names(test)
glimpse(test)
